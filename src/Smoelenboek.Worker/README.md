...
2. **Map** — converts each user to a Medewerker object matching the [medewerker schema](https://github.com/open-objecten/objecttypes/blob/main/community-concepts/Medewerker/medewerker-schema.json), resolving each Entra user's `department` to a `naam` by matching against Afdeling objects already present in Open Objects
...
| [Open Objects API](https://github.com/maykinmedia/objects-api) | running and configured with the 'Medewerker', 'Afdeling' and 'Groep' object types |
...
| `OpenObjects__Token` | API token sent as `Authorization: Token <value>`. To obtain: in the Objects admin UI, create a **Permission** scoped to the Medewerker (and Afdeling and Groep, for read access) object types, then create a **Token Authorization** for that permission and copy the resulting token string here. | **Yes** |
...
| `OpenObjects__GroepObjectTypeUrl` | URL of the Groep object type — currently not used by the sync | No |
| `OpenObjects__GroepObjectTypeVersion` | Not currently used | No |
