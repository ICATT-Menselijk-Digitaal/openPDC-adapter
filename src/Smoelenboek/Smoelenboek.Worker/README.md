...
2. **Map** — converts each user to a Medewerker object, resolving each Entra user's `department` to a `naam` by matching against Afdeling objects already present in Open Objects
...
Afdeling objects themselves are **not** synced by this adapter — they're expected to already exist in Open Objects (entered manually) and are only read to resolve the `identificatie` reference on each Medewerker.
...
| [Open Objects API](https://github.com/maykinmedia/objects-api) | running |
| the following objecttypes are registered in OpenObjects | [medewerker schema](https://github.com/open-objecten/objecttypes/blob/main/community-concepts/Medewerker/medewerker-schema.json), [afdeling schema](https://github.com/open-objecten/objecttypes/blob/main/community-concepts/Afdeling%20en%20Groep/afdeling-schema.json)|
| Manually add some afdelingen to OpenObjects | {"naam": "Beleid Leefomgeving", "identificatie": "beleid-leefomgeving"}, {"naam": "Facilitaire Zaken en Frontoffice", "identificatie": "facilitaire-zaken-en-frontoffice"}, {"naam": "Directie", "identificatie": "directie"} |
...
All values can be set as environment variables, or user secrets, or in `appsettings.json`. Environment variables take precedence.
...
| `OpenObjects__Token` | API token sent as `Authorization: Token <value>`. To obtain: in the Objects admin UI, create a **Permission** scoped to the Medewerker and Afdeling object types, then create a **Token Authorization** for that permission and copy the resulting token string here. | **Yes** |
...
| `OpenObjects__AfdelingObjectTypeUrl` | URL of the Afdeling object type — read-only, Afdeling objects are entered manually and never written by this adapter | **Yes** |
| `OpenObjects__AfdelingObjectTypeVersion` | Not currently used (Afdeling is only ever read, never written) | No |

** Note, make sure you use the uuid's for the objecttypes that you just created. **
